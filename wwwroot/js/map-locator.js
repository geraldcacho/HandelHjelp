// Azure Maps Order Locator (version 1.0-rc.2)
...
class orderlocator {
    ...
    options = null;                        // Options for the order locator.
    ...
    isochrone = {
        ...
        radiusDataSource: new atlas.source.DataSource()
    };
    ...
    */
    constructor(map, options = {}) {
        ...
        this.options = {
            ...
            maxSearchResults: 20,           // Default max search results to show in list. (max 9).
    ...
    initializeFilters() {
        ...
        this.getSupportedTypes().then((supportedTypes) => {
            ...
            const subCategoryTemplate = document.getElementById('template-filters-tag').innerHTML;

            var html = '';
            for (const category of supportedTypes) {

                var innerHtml = '';
                for (const subCategory of category.subCategories) {
                    innerHtml += subCategoryTemplate
                        .replaceAll('{id}', subCategory.id)
                        .replace('{name}', subCategory.name);
                }

                html += categoryTemplate
                    .replaceAll('{orderTypeId}', category.OrderTypeId)
                    .replace('{name}', category.name)
                    .replace('{expanded}', html == '' ? 'true' : 'false')
                    .replace('{show}', html == '' ? 'show' : '')
                    .replace('{subCategory}', innerHtml);
            }
    ...
    initializeIsochroneLayers() {
        ...
        // Add a layer for the radius isochrone.
        const radiusLineLayer = new atlas.layer.PolygonLayer(this.isochrone.radiusDataSource, null, {
            fillColor: 'rgba(0, 200, 200, 0.5)'
        });
        this.map.sources.add(this.isochrone.radiusDataSource);
        this.map.layers.add(radiusLineLayer);
    }
    ...
    /**
     * Handels the store cick event, centers the map and opens the store popup.
     * @param {string} id
     */
    storeClicked(id) {
    ...
    */
    fuzzySearch(query) {
        ...
        Promise.all([
            ...
            html += '<li><h6 class="dropdown-header">Orders</h6></li>';
            ...
            for (const order of response[1]) {
                html += this.search.autocomplete.storesTemplate
                    ...
                    .replace('{CountryName}', order.CountryName);
            }
    ...
    */
    ...
    showNearbyStores(coordinates = null) {
        ...
        this.map.markers.add(new atlas.HtmlMarker({
            color: 'DodgerBlue',
            htmlContent: '<i class="bi bi-dot" style="font-size: 3rem; color: {color};"></i>',
        ...
        // Get nearby stores.
        this.getNearbyStores(coordinates, this.search.features).then((orders) => {
            ...
            //orders.sort((a, b) => a['distanceInKm'] - b['distanceInKm']);
            ...
            orders.forEach((order) => {
                ...
                if (count++ < this.options.maxSearchResults) {
                    html += this.search.resultsTemplate
                        ...
                        .replace('{orderId}', order.orderId)
                        //.replace('{imageUrl}', store.imageUrl)
                        //.replace('{name}', store.name + 'emil')
                        //.replace('{address}', store.address.streetAddressLine1)
                        .replace('{city}', order.city)
                        .replace('{countryName}', order.countryName)
                        .replace('{distanceInKm}', order.distanceInKm.toFixed(1))
                        .replace('{distanceInMile}', this.convertToMiles(order.distanceInKm).toFixed(1))
                        //.replace('{orderNumber}', store.orderNumber)
                        .replace('{longitude}', order.latitude)
                        .replace('{latitude}', order.longitude);
        ...
        this.drawRadiusIsochrone(coordinates, this.options.rangeInKm);

    }

    async drawRadiusIsochrone(position, distanceInKM) {
        try {
            // Add the first coordinate to the end of the array
            this.isochrone.radiusDataSource.clear();
            this.isochrone.radiusDataSource.add(new atlas.data.Feature(new atlas.data.Point([position[0], position[1]]), {
                subType: "Circle",
                radius: 700 * distanceInKM
            }));
        } catch (error) {
    ...
    */
    ...
    async getStoresByCountry(country) {
        ...
        const response = await fetch(`/api/orders/search?country=${country}`);
    ...
    async getSupportedTypes() {
        const response = await fetch('/api/orders/supportedtypes');
    ...
    async getNearbyStores(position, filters = []) {
        ...
        const response = await fetch(`/api/orders/search?latitude=${position[1]}&longitude=${position[0]}&rangeInKm=${this.options.rangeInKm}&tags=${filters}`);
    ...
    async searchStores(query, limit = 5) {
        ...
        const response = await fetch(`/api/orders/search?query=${query}&limit=${limit}`);
